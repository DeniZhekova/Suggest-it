...
exports.createPostValidator=(req,res,next)=>{
    ...
    check('body',"Should be of 4 to 3000 length").isLength({min:4,max:30000})
    // check for errors
...
exports.signupValidator=(req,res,next)=>{
    ...
    check('password',req.body.password).isLength({min:6}).withMessage("Password must have at least 6 chars")
    .matches(/\d/)
    const errors = validationResult(req);
    if (!errors.isEmpty()) {
        return res.status(422).json({ errors: errors.array() });
