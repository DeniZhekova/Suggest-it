...
export const create= async (userId,token,post)=>{
    return await axios.post(`/api/post/new/${userId}`,post,{ headers: {"Authorization" : `Bearer ${token}`} })
        .then(res=> res.data)
        .catch(err => err);
...
export const pageList = async page => {
    return await axios.get(`/api/post/?page=${page}`)
        .then(res=>{
            console.log(res);
            return res.data;
        }).catch(err=> {
...
export const listByUser = async (userId, token) => {
    return await axios
        .get(`/api/post/by/${userId}`,{ headers: {"Authorization" : `Bearer ${token}`}})
