...
const Navbar = ({history}) => {
    ...
    return (
                <nav className="navbar navbar-expand-lg navbar-dark scrolling-navbar" style={{backgroundColor:'#0E2F56'}}>
                    <NavLink to='/' className="navbar-brand"><img alt='brand' src={Logo} style={{height:'100px',maxWidth:'140px',marginTop: '-30px',marginBottom:'-40px'}}></img></NavLink>
                    ...
                    <div className="collapse navbar-collapse" id="navbarSupportedContent">
                        ...
                        {isAuthenticated() && (
                            <li className='nav-item'>
                                <span className="navbar-text ml-auto white-text">
                                   <NavLink className="nav-link" activeClassName="line" to={`/user/${id}`}>{name}</NavLink>
                                </span>
                            </li>
                        )}
