...
class Suggest extends Component {
    ...
    componentDidMount(){
        this.suggestData=new FormData(); // browser api by default in chrome
    ...
    clickSubmit=(event)=>{
        ...
        create(userId, token, this.suggestData).then(data => {
            if (data.error) {
            this.setState({ error: data.error,loading:false });
            } 
            else {
            this.setState({loading:false,redirect:'true',title:'',body:'',photo:'',error:''});
            }
        });
        }
